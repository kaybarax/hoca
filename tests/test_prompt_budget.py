...
REVIEWER_SCRIPT_CHAR_BUDGET = 9000
...
def test_direct_reviewer_prompt_keeps_report_rules_without_long_rubric() -> None:
    ...
    review_task_start = script.index('REVIEW_TASK="Review the current repository changes')
    ...
    assert "Test summary contents:" in script
    ...
    assert "never create temporary" in script
    assert "never use rm -rf, rm -Rf, or other" in script
    assert "Never place shell commands inside delegated task or subagent instructions" in script
