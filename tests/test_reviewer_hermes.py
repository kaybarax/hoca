...
def test_run_reviewer_hermes_profile_mode_invokes_hermes_and_uses_report(
    ...
    hermes.write_text(
        ...
        "shift\n"
        'while [[ $# -gt 0 ]]; do case "$1" in --query|-q) PROMPT="${2:-}"; shift 2;; --max-turns) [[ "${2:-}" == "20" ]] || { echo "wrong max turns" >&2; exit 2; }; shift 2;; --model) [[ "${2:-}" == */* ]] || { echo "missing model override" >&2; exit 2; }; shift 2;; *) shift;; esac; done\n'
...
def test_run_reviewer_hermes_normalizes_multiline_pr_notes(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    fake_bin = tmp_path / "bin"
    fake_bin.mkdir()
    ...
    hermes.write_text(
        ...
        'RUN_DIR="${HERMES_TEST_RUN_DIR:?}"\n'
        'mkdir -p "$RUN_DIR/reviews"\n'
        'cat > "$RUN_DIR/reviews/review-report-1.json" <<EOF\n'
    ...
    assert report.pr_notes["suggested_pr_body"] == ["Summary - Typed env config"]
