...
class TestMonitorProcessStream:
    ...
    def test_dangerous_text_in_partial_agent_thought_stream_is_ignored(self, tmp_path: Path):
        import io

        partial_thought = (
            '{"id":"event-1","source":"agent","thought":[{"type":"text","text":""}],'
            '"reasoning_content":"Existing script text mentions rm -rf."'
        )
        stream = io.StringIO(f"reading\n{partial_thought}\ndone\n")
        result = monitor_process_stream(
            stream,
