...
class FakeStdoutReviewProcess:
    def __init__(self, command, *, stdout_path: Path):
        self.command = tuple(command)
        self.returncode: int | None = None
        stdout_path.write_text(
            ...
            "```json\n"
            '{"schema_version":1,"run_id":"run-test","round":1,"role":"reviewer",'
            '"verdict":"LGTM","findings":[],"pr_notes":{"summary":["Recovered from stdout."],'
            '"known_followups":[]}}\n'
            "```\n",
            encoding="utf-8",
        )
...
class FakeGracefulReviewProcess:
    def __init__(self, command, *, report_path: Path, review_text_path: Path):
        ...
        report_path.write_text(
            '{"schema_version":1,"run_id":"run-test","round":1,"role":"reviewer",'
            '"verdict":"LGTM","findings":[],"pr_notes":{"summary":["Report already written."],'
            '"known_followups":[]}}\n',
            encoding="utf-8",
        )
...
class FakeSlowShutdownReviewProcess:
    def __init__(self, command, *, report_path: Path, review_text_path: Path):
        self.command = tuple(command)
        self.returncode: int | None = None
        self.terminate_calls = 0
        ...
        report_path.write_text(
            '{"schema_version":1,"run_id":"run-test","round":1,"role":"reviewer",'
            '"verdict":"LGTM","findings":[],"pr_notes":{"summary":["Slow shutdown."],'
            '"known_followups":[]}}\n',
            encoding="utf-8",
        )
...
def test_run_reviewer_direct_keeps_lgtm_when_wrapper_exits_nonzero(
    tmp_path: Path, monkeypatch
) -> None:
    project = tmp_path / "project"
    init_repo(project)
...
def test_run_reviewer_direct_recovers_fenced_report_from_stdout(
    tmp_path: Path, monkeypatch
) -> None:
    project = tmp_path / "project"
    init_repo(project)
    ...
    def fake_popen(command, **kwargs):
        return FakeStdoutReviewProcess(
            command,
...
def test_run_reviewer_direct_waits_for_report_before_terminating(
    tmp_path: Path, monkeypatch
) -> None:
    project = tmp_path / "project"
    init_repo(project)
    ...
    task_spec_path = run_dir / "task-spec.json"
    task_spec_path.write_text(sample_task_spec(repo_root=str(project)).to_json(), encoding="utf-8")
    processes: list[FakeGracefulReviewProcess] = []

    def fake_popen(command, **kwargs):
        process = FakeGracefulReviewProcess(
            command,
...
def test_run_reviewer_direct_treats_self_termination_after_report_as_success(
    tmp_path: Path, monkeypatch
) -> None:
    project = tmp_path / "project"
    init_repo(project)
    ...
    task_spec_path = run_dir / "task-spec.json"
    task_spec_path.write_text(sample_task_spec(repo_root=str(project)).to_json(), encoding="utf-8")
    processes: list[FakeSlowShutdownReviewProcess] = []

    def fake_popen(command, **kwargs):
        process = FakeSlowShutdownReviewProcess(
            command,
            report_path=review_report_path(run_dir, 1),
            review_text_path=run_dir / "openhands-review.txt",
        )
    ...
    assert result.exit_code == 0
    assert processes and processes[0].terminate_calls >= 1
    assert HocaReviewReport.from_json(result.review_report_path.read_text()).verdict == "LGTM"
